						...
						{isLoading ?
							...
							: values ? (values.map((item, index) =>
								<div key={index} className={classes.tableInfoCampaigns}>
									...
									<div style={{backgroundColor: effortColors[item.effort]}} className={classes.effort}/>
									<div className={classes.campaignLogo}>
										{item.campaign_logo ? <img src={item.campaign_logo} alt='photo' className={`${classes.avatarComp} ${classes.photo}`}/>
									...
									<div className={classes.rightSection}>
										...
										<div className={classes.mainStatusWrap}>
											<div className={classes.statusWrap} style={{backgroundColor: statusBackColors[item.status.split('-').join('')]}}>
												<div style={{backgroundColor: statusColors[item.status.split('-').join('')]}} className={classes.statusDot}/>
												<div style={{color: statusColors[item.status.split('-').join('')]}}>{item.status}</div>
											</div>
										</div>

										<div className={classes.teamLead}>
											{item.team_lead[0].profile_picture ? <img src={item.team_lead[0].profile_picture} alt={'logo'}
